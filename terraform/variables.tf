...
variable "cloudflare_token" {
  type        = string
  description = "My Profile -> API Tokens"
  sensitive   = true
}

variable "cloudflare_zone_id" {
  type        = string
  description = "Cloudflare Zone ID. Can be found in the Overview tab of the domain."

  validation {
    condition     = can(regex("^[0-9a-f]{32}$", var.cloudflare_zone_id))
    error_message = "The cloudflare_zone_id value must be a valid Cloudflare Zone ID."
  }
}
